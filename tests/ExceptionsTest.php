...
use SmartOver\ApiResponse\Exceptions\ExceptionThrower;
...
class ExceptionsTest extends TestCase {
	...
	public function testDefaultException() {
		$request
			= Request::createFromBase( \Symfony\Component\HttpFoundation\Request::create(
			"uri",
			"get"
		) );
	...
	public function testRequiredParameterException() {
		...
		$exception = new ExceptionHandler();
		$except    = $exception->render( $request,
			new RequiredParameterException( 'foo' ) );
		$decoded   = json_decode( $except->getContent() );
	...
	public function testGenericException() {
		...
		$exception = new ExceptionHandler();
		$except    = $exception->render( $request,
			new GenericException( 405, ResponseCode::ERR001, 'message',
				[ 'foo' => 'bar' ] ) );
		$decoded   = json_decode( $except->getContent() );
	...
	public function testThrowException() {
		...
		ExceptionThrower::genericWithMessage( 'Custom message' );
